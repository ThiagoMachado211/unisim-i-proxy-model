...
# ============================================================
# PIPELINE COMPLETA PARA CONSTRUÇÃO DE PROXY MODEL
# ============================================================
#
# Este script reúne, em um único arquivo, todo o fluxo de:
#
# 1. leitura/inventário dos casos do simulador
# 2. extração das séries temporais
# 3. controle de qualidade e padronização
# 4. montagem do dataset tabular
# 5. leitura dos parâmetros dos casos
# 6. construção do dataset final de machine learning
# 7. treinamento do proxy
# 8. validação cruzada por caso
# 9. geração dos arquivos de saída
# 10. geração de figuras de comparação entre simulador e proxy
#
# O objetivo é que a equipe receba as pastas CASE_XX vindas do
# simulador, execute este arquivo e obtenha:
#
# - modelos treinados
# - métricas
# - previsões
...
# ------------------------------------------------------------
# IMPORTAÇÃO DAS ETAPAS JÁ EXISTENTES NO PROJETO
# ------------------------------------------------------------
#
# Estas funções já foram implementadas no projeto principal e
# são reaproveitadas aqui para evitar duplicação de código.
#
# Cada uma tem um papel específico:
#
# - run_inventory:
#       faz o inventário das pastas CASE_XX
#
# - run_extract_summary:
#       extrai as séries temporais do simulador
#
# - run_qc:
#       executa controle de qualidade e padronização
#
# - run_build_tabular:
#       monta o dataset tabular
#
# - run_parse_cases:
#       lê o README e extrai os parâmetros dos casos
#
# - run_build_ml:
#       gera o dataset final de machine learning
# ------------------------------------------------------------
...
def build_feature_target_columns(df: pd.DataFrame) -> tuple[list[str], list[str]]:
    """
    Separa automaticamente as colunas em:
    - features (entradas)
    - targets (saídas)

    Features:
        colunas relacionadas aos parâmetros dos injetores
        + t_days

    Targets:
        colunas que representam as respostas do reservatório
    """
    feature_cols = [
        c
        for c in df.columns
...
def compute_global_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    """
    Calcula métricas globais considerando todas as variáveis juntas.
...
def compute_metrics_by_variable(
    y_true_df: pd.DataFrame,
    y_pred_df: pd.DataFrame,
) -> pd.DataFrame:
    """
    Calcula métricas separadamente para cada variável de saída.

    Isso é útil para identificar:
    - quais variáveis o proxy prevê melhor
    - quais variáveis ainda apresentam maior erro
    """
    rows = []
        ...
        rows.append(
            {
                ...
                "MAPE_pct": float(
                    np.mean(np.abs(yt - yp) / np.maximum(np.abs(yt), 1e-9)) * 100.0
                ),
            }
        )
...
def compute_metrics_by_case(
    case_ids: np.ndarray,
    y_true: np.ndarray,
    y_pred: np.ndarray,
) -> pd.DataFrame:
    """
    Calcula métricas separadamente para cada caso de simulação.
    """
    rows = []
    ...
    for case_id in sorted(np.unique(case_ids)):
        ...
        rows.append(
            {
                "case_id": int(case_id),
                **compute_global_metrics(y_true[mask], y_pred[mask]),
            }
        )
...
def generate_plots(
    ...
    """
    Gera figuras de comparação entre simulador e proxy.

    Figuras geradas:
    - curvas real vs proxy
    - scatter real vs predito
    - erro absoluto médio ao longo do tempo
    """
    figures_dir = ensure_dir(figures_dir)

    preferred_targets = [
        "WOPR_NA1A",  # produção de óleo
        "WWPR_NA1A",  # produção de água
        "WWIR_I1",    # taxa de injeção
        "WBHP_NA1A",  # pressão
    ]

    chosen_targets = [c for c in preferred_targets if c in target_cols]
    if not chosen_targets:
        chosen_targets = target_cols[:4]

    ordered_cases = case_metrics["case_id"].tolist()

    if len(ordered_cases) >= 3:
        cases_to_plot = [
            ordered_cases[0],                    # pior caso
            ordered_cases[len(ordered_cases)//2],  # caso mediano
            ordered_cases[-1],                   # melhor caso
        ]
    else:
        cases_to_plot = ordered_cases

    # --------------------------------------------------------
    # 1. Curvas real vs proxy
    # --------------------------------------------------------
    ...
    for target in chosen_targets:
        tmp[f"ae_{target}"] = np.abs(
            df_eval[f"{target}__true"].values - df_eval[f"{target}__pred"].values
        )
...
def train_and_validate_proxy(
    project_root: str | Path = ".",
    model_name: str = "rf_tabular",
    output_dir: str | Path = "outputs/rf_tabular",
    n_splits: int = 5,
    random_state: int = 42,
) -> dict:
    """
    Treina e valida o proxy com GroupKFold por case_id.

    Este é o coração da parte de machine learning:
    - lê o dataset final
    ...
    # Os grupos são os case_id
    groups = df["case_id"].values

    gkf = GroupKFold(n_splits=n_splits)

    # Estruturas para consolidar resultados finais
    all_true = []
    all_pred = []
    all_case_ids = []
    all_t_days = []
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        # Constrói o modelo correspondente ao nome solicitado
        ...
        fold_rows.append(
            {
                ...
                **compute_global_metrics(y_true_df.values, y_pred_df.values),
            }
        )
    ...
    # Salva as previsões completas
    df_eval = pd.DataFrame(
        {
            "case_id": case_ids_all,
            "date": all_dates,
            "t_days": all_t_days,
        }
    )
...
def run_full_pipeline(
    ...
    """
    Executa a pipeline completa do início ao fim.
    """
    ...
    print("[STEP 7] Treino do proxy e geração das saídas")
    result = train_and_validate_proxy(
...
def parse_args():
    ...
    Exemplos de uso:
    python full_proxy_pipeline_commented.py --model rf_tabular
    python full_proxy_pipeline_commented.py --model xgb_tabular
    python full_proxy_pipeline_commented.py --model mlp_tabular
    """
    parser = argparse.ArgumentParser(
        description=(
            "Pipeline completa: leitura dos dados do simulador, "
            "preparação do dataset, treino do proxy e geração de arquivos de saída."
        )
    )
    ...
    parser.add_argument(
        "--model",
        default="rf_tabular",
        choices=["rf_tabular", "xgb_tabular", "mlp_tabular"],
        help="Modelo de proxy a treinar.",
    )

    parser.add_argument(
        "--output-dir",
        default=None,
        help="Diretório de saída. Se omitido, usa outputs/<model>.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Número de folds do GroupKFold.",
    )
